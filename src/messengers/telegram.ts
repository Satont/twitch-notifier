...
class Telegram extends IService {
  ...
  scenes: { [x: string]: BaseScene<any> } = {
    follow: new BaseScene('follow', {
      ...
      enterHandlers: [
        async (ctx) => {
          ...
          if (!channel) return ctx.reply('Enter streamer username you want to follow')
    ...
    unfollow: new BaseScene('unfollow', {
      ...
      enterHandlers: [
        async (ctx) => {
          const channel = ctx.message.text.substring(ctx.message.entities[0].length).trim()
          if (!channel) return ctx.reply('Enter streamer username you want to unfollow')
  ...
  protected async loadMiddlewares(): Promise<void> {
    ...
    this.bot.use(async (ctx, next) => {
      ...
      info(
        `Telegram | ` +
        `New message from ${ctx.from.username} [${ctx.from.id}], message: ${ctx.message.text}`
      )
      const [user] =
        await User.findOrCreate(
          { where: { id: ctx.from.id, service }, defaults: { follows: [], service } }
        )
      ctx.userDb = user
      next()
    })
  ...
  protected async loadCommands() {
    ...
    this.bot.command(
      ['start', 'help'],
      ({ reply }) => reply(`Hi! I will notify you about the beginning of the broadcasts on Twitch.`)
    )
    ...
    this.bot.command('follows', async (ctx: Context) => {
      ...
      if (isBoolean(channels)) {
        this.sendMessage({
          target: ctx.from.id,
          message: `You aren't followed to someone`
        })
      } else {
        this.sendMessage({
          target: ctx.from.id,
          message: `You are followed to ${channels.join(', ')}`
        })
      }
    })
    this.bot.command('live', async (ctx: Context) => {
      ...
      if (isBoolean(channels)) {
        this.sendMessage({
          target: ctx.from.id,
          message: `There is no channels currently online`
        })
      } else {
        this.sendMessage({
          target: ctx.from.id,
    ...
    this.bot.command('watch_game_change', async (ctx: Context) => {
      ...
      if (result) {
        this.sendMessage({ target: ctx.from.id, message: 'Watching game change was enabled'})
      } else {
        this.sendMessage({ target: ctx.from.id, message: 'Watching game change was disabled'})
      }
    })
  }
}
