...
// The ChatSettingsFunc type is an adapter to allow the use of ordinary
// function as ChatSettings mutator.
type ChatSettingsFunc func(context.Context, *ent.ChatSettingsMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ChatSettingsFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ChatSettingsMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ChatSettingsMutation", m)
}
