...
func (csc *ChatSettingsCreate) SetChatID(u uuid.UUID) *ChatSettingsCreate {
	csc.mutation.SetChatID(u)
	return csc
}
...
func (csc *ChatSettingsCreate) SetChat(c *Chat) *ChatSettingsCreate {
	return csc.SetChatID(c.ID)
}
...
func (csc *ChatSettingsCreate) check() error {
	...
	if _, ok := csc.mutation.ChatID(); !ok {
		return &ValidationError{Name: "chat_id", err: errors.New(`ent: missing required field "ChatSettings.chat_id"`)}
	}
	if _, ok := csc.mutation.ChatID(); !ok {
		return &ValidationError{Name: "chat", err: errors.New(`ent: missing required edge "ChatSettings.chat"`)}
	}
	return nil
}
...
func (csc *ChatSettingsCreate) createSpec() (*ChatSettings, *sqlgraph.CreateSpec) {
	...
	if nodes := csc.mutation.ChatIDs(); len(nodes) > 0 {
