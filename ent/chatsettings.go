...
func (e ChatSettingsEdges) OwnerOrErr() (*Chat, error) {
	if e.loadedTypes[0] {
		if e.Owner == nil {
...
func (*ChatSettings) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case chatsettings.ForeignKeys[0]: // chat_chat_settings
...
func (cs *ChatSettings) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case chatsettings.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field chat_chat_settings", values[i])
			} else if value.Valid {
				cs.chat_chat_settings = new(uuid.UUID)
				*cs.chat_chat_settings = *value.S.(*uuid.UUID)
			}
		}
	}
	return nil
...
// QueryOwner queries the "owner" edge of the ChatSettings entity.
func (cs *ChatSettings) QueryOwner() *ChatQuery {
	return NewChatSettingsClient(cs.config).QueryOwner(cs)
}
