...
// HasChatSettings applies the HasEdge predicate on the "chat_settings" edge.
func HasChatSettings() predicate.Chat {
	return predicate.Chat(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.Edge(sqlgraph.O2O, false, ChatSettingsTable, ChatSettingsColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasChatSettingsWith applies the HasEdge predicate on the "chat_settings" edge with a given conditions (other predicates).
func HasChatSettingsWith(preds ...predicate.ChatSettings) predicate.Chat {
	return predicate.Chat(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(ChatSettingsInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2O, false, ChatSettingsTable, ChatSettingsColumn),
		)
