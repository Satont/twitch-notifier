...
type ChatQuery struct {
	...
	withSettings *ChatSettingsQuery
...
func (cq *ChatQuery) QuerySettings() *ChatSettingsQuery {
...
func (cq *ChatQuery) Clone() *ChatQuery {
	...
	return &ChatQuery{
		...
		withSettings: cq.withSettings.Clone(),
...
func (cq *ChatQuery) WithSettings(opts ...func(*ChatSettingsQuery)) *ChatQuery {
	query := (&ChatSettingsClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withSettings = query
	return cq
}
...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := cq.withSettings; query != nil {
		if err := cq.loadSettings(ctx, query, nodes, nil,
			func(n *Chat, e *ChatSettings) { n.Edges.Settings = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *ChatQuery) loadSettings(ctx context.Context, query *ChatSettingsQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *ChatSettings)) error {
	...
	query.Where(predicate.ChatSettings(func(s *sql.Selector) {
		s.Where(sql.InValues(chat.SettingsColumn, fks...))
	}))
