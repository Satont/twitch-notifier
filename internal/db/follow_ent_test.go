...
func TestFollowService_Create(t *testing.T) {
	...
	newChat, err := chService.Create(ctx, "1", db_models2.ChatServiceTelegram)
	assert.NoError(t, err)

	newChannel, err := channelsService.Create(ctx, "1", db_models2.ChannelServiceTwitch)
	assert.NoError(t, err)
...
func TestFollowService_GetByChannelID(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		ch, err := channelsService.Create(
			ctx,
			fmt.Sprintf("%v", i),
			db_models2.ChannelServiceTwitch,
		)
		assert.NoError(t, err)
